/**
 * Test script for Google Translate Provider
 *
 * Usage:
 * 1. Create a .env file in the project root with your GOOGLE_TRANSLATE_API_KEY
 * 2. Run: npx tsx test/test-google.ts
 */

import 'dotenv/config';
import { GoogleTranslateProvider } from '../src/translations/utils/google-translate-provider.js';

async function testGoogleProvider() {
  console.log('🧪 Testing Google Translate Provider\n');

  const apiKey = process.env.GOOGLE_TRANSLATE_API_KEY;

  if (!apiKey) {
    console.error('❌ Error: GOOGLE_TRANSLATE_API_KEY not found in environment variables');
    console.error('Please create a .env file with: GOOGLE_TRANSLATE_API_KEY=your_key_here');
    process.exit(1);
  }

  const provider = new GoogleTranslateProvider();

  console.log('🔑 API: Google Cloud Translation API v2');
  console.log('🌐 Endpoint: translation.googleapis.com\n');
  ...
  // Test 3: Translation without source language (auto-detect)
  console.log('Test 3: Auto-detect source language');
  console.log('─'.repeat(50));
  try {
    const result3 = await provider.translate({
      text: 'Bonjour le monde',
      sourceLang: 'fr', // Google requires source lang
      targetLang: 'en',
      apiKey
    });
    console.log('✅ Source: "Bonjour le monde" (French)');
